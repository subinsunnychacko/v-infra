...
const Testimonials = () => {
  ...
  const testimonials = [
    {
      id: 1, name: 'Rajesh Kumar', role: 'Project Director', location: 'Noida, UP',
    ...
    {
      id: 2, name: 'Suresh Patel', role: 'Site Engineer', location: 'Gurgaon, HR',
    ...
    {
      id: 3, name: 'Priya Sharma', role: 'Structural Consultant', location: 'New Delhi',
    ...
    {
      id: 4, name: 'Amit Verma', role: 'Development Head', location: 'Delhi NCR',
    ...
    {
      id: 5, name: 'Neha Gupta', role: 'Project Director', location: 'Chandigarh',
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      gsap.fromTo('.testimonials-slider', { opacity: 0, y: 60 }, { opacity: 1, y: 0, duration: 1, ease: 'power3.out', scrollTrigger: { trigger: '.testimonials-slider', start: 'top 80%' } });
    }, sectionRef);
    return () => ctx.revert();
  }, []);

  return (
    <section ref={sectionRef} className="py-24 lg:py-32 relative overflow-hidden" style={{ backgroundColor: currentTheme.colors.primary }}>
      <div className="absolute top-0 right-0 w-96 h-96 rounded-full blur-3xl opacity-20" style={{ backgroundColor: currentTheme.colors.secondary }} />
      <div className="absolute bottom-0 left-0 w-96 h-96 rounded-full blur-3xl opacity-10" style={{ backgroundColor: currentTheme.colors.accent2 }} />
      <Quote className="absolute top-20 left-10 opacity-5" size={300} style={{ color: currentTheme.colors.textOnDark }} />

      <div className="max-w-7xl mx-auto px-6 lg:px-8 relative z-10">
        <div className="testimonials-header text-center mb-16">
          <span className="inline-block font-body text-sm font-semibold tracking-wider uppercase mb-4" style={{ color: currentTheme.colors.secondary }}>Testimonials</span>
          <h2 className="font-heading text-3xl md:text-4xl lg:text-5xl font-bold mb-4" style={{ color: currentTheme.colors.textOnDark }}>What Our Clients Say</h2>
          <CurvedUnderline width="160" className="mx-auto mb-6" />
          <p className="font-body text-lg max-w-2xl mx-auto" style={{ color: `${currentTheme.colors.textOnDark}cc` }}>Don&apos;t just take our word for it. Here&apos;s what our satisfied customers have to say about their experience with V Infra Engineers.</p>
        </div>
        ...
        <style jsx global>{`
          .swiper-bullet-custom { width: 12px; height: 12px; background: ${currentTheme.colors.textOnDark}40; border-radius: 50%; display: inline-block; margin: 0 6px; cursor: pointer; transition: all 0.3s ease; }
          .swiper-bullet-active-custom { background: ${currentTheme.colors.secondary}; width: 32px; border-radius: 6px; }
