...
const Hero = () => {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      gsap.set(['.hero-title', '.hero-subtitle', '.hero-text', '.hero-buttons', '.hero-stats'], {
        opacity: 0,
        y: 50,
      });
      ...
      tl.to('.hero-badge', { opacity: 1, x: 0, duration: 0.6 }, 0.3)
        ...
        .to('.floating-card', { opacity: 1, y: 0, duration: 0.6, stagger: 0.15 }, 1.2);
    ...
    >
      ...
      <div className="absolute inset-0 opacity-30">
        <div
          ...
          style={{ backgroundColor: currentTheme.colors.secondary, opacity: 0.2 }}
        />
        <div
          className="absolute bottom-0 left-0 w-96 h-96 rounded-full blur-3xl"
          style={{ backgroundColor: currentTheme.colors.accent2, opacity: 0.2 }}
        />
      </div>

      {/* Beam Decoration */}
      <div className="beam-element absolute top-0 right-1/4 w-8 h-32 rounded-b-full opacity-60"
        style={{ backgroundColor: currentTheme.colors.secondary }}
      />
      <div className="beam-element absolute top-0 right-1/3 w-6 h-24 rounded-b-full opacity-40"
      ...
      <div className="relative max-w-7xl mx-auto px-6 lg:px-8 py-20">
        <div className="grid lg:grid-cols-2 gap-12 lg:gap-20 items-center">
          ...
          <div ref={contentRef} className="relative z-10">
            ...
            >
              <Star size={16} style={{ color: currentTheme.colors.secondary }} fill={currentTheme.colors.secondary} />
              <span
            ...
            {/* Title */}
            <h1 className="hero-title font-heading text-4xl md:text-5xl lg:text-6xl font-bold leading-tight mb-2"
              style={{ color: currentTheme.colors.heading }}
            >
              The Underground
            </h1>
            <h2 className="hero-subtitle font-heading text-4xl md:text-5xl lg:text-6xl font-bold leading-tight mb-4"
              style={{ color: currentTheme.colors.secondary }}
            >
            ...
            <p className="hero-text font-body text-lg md:text-xl leading-relaxed mb-8 max-w-xl"
              style={{ color: currentTheme.colors.body }}
            >
              Specialized engineering services for diaphragm wall construction, deep foundations, and underground structures. 
              Precision engineering, safety-first approach, and on-time project delivery since 2014.
            </p>
            ...
            {/* Stats */}
            <div ref={statsRef} className="hero-stats flex flex-wrap gap-8 md:gap-12">
          ...
          <div ref={imageRef} className="relative lg:h-[600px]">
            ...
            >
              <div className="flex items-center gap-3">
                ...
                >
                  <Star size={24} style={{ color: currentTheme.colors.secondary }} fill={currentTheme.colors.secondary} />
                </div>
                <div>
                  <div className="flex items-center gap-1 mb-1">
                    {[...Array(5)].map((_, i) => (
                      <Star key={i} size={14} style={{ color: currentTheme.colors.secondary }} fill={currentTheme.colors.secondary} />
                    ))}
                  </div>
                  <p className="font-body text-sm font-medium" style={{ color: currentTheme.colors.heading }}>
                    4.9/5 Rating
                  </p>
                  <p className="font-body text-xs" style={{ color: currentTheme.colors.muted }}>
                    500+ Reviews
                  </p>
                </div>
              </div>
            ...
            <div className="absolute -bottom-6 left-1/4 flex gap-2">
              {[currentTheme.colors.primary, currentTheme.colors.secondary, currentTheme.colors.accent1, currentTheme.colors.accent2].map((color, i) => (
                <div
                  key={i}
      ...
      <div className="absolute bottom-8 left-1/2 -translate-x-1/2 flex flex-col items-center gap-2">
        <span className="font-body text-xs tracking-wider" style={{ color: currentTheme.colors.muted }}>
          SCROLL
        </span>
        <div
